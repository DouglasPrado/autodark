...
Para cada doc em `docs/blueprint/`, `docs/backend/`, `docs/frontend/`, `docs/shared/`, e `docs/business/` (se existir):
...
**Docs:** `docs/blueprint/` (O QUE) → `docs/backend/` (COMO backend) → `docs/frontend/` (COMO frontend, com `shared/` + per-client) → `docs/shared/` (glossario, mappings) [→ `docs/business/` (modelo de negocio, se existir)]
